...
from sklearn.metrics import f1_score, accuracy_score, ndcg_score, roc_auc_score, recall_score
...
class Evaluator():
    ...
    def cal_recl(self, y_true, y_pred):
        return recall_score(y_true, y_pred)

    def cal_multi(self, y_label, y_pred):
        macro_f1 = f1_score(y_label, y_pred, average='macro')
        micro_f1 = f1_score(y_label, y_pred, average='micro')
        # 二分类的micro-F1 score和Accuracy的值相等
        acc = accuracy_score(y_label, y_pred)
        recall = recall_score(y_label, y_pred)
        auc = roc_auc_score(y_label, y_pred)
        return dict(Macro_f1=macro_f1, Micro_f1=micro_f1, Acc=acc, AUC=auc, Recall=recall)
