...
def train(features, adj):
    ...
    # loss_train = F.nll_loss(output[idx_train], labels[idx_train]) + loss_dis
...
for epoch_gen in range(args.epochs_gen):
    ...
    num_real = len(torch.cat((majority[range_train_maj], minority[range_train_min])))
    ...
    if val_loss_pr < f1_val:
        val_loss_pr = f1_val
        adj_new_output = adj_new
    else:
        num_stop += 1

    if num_stop == eary_stop:
        print("eary stop !!!")
        break
...
torch.save(adj_new_output.cpu(), "adj_" + "service" + "_new.pth")
torch.save(adj_real, "adj_real.pth")
