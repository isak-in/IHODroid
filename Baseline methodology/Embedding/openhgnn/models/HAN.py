...
class HAN(BaseModel):
    ...
    def forward(self, g, h_dict):
        ...
        # for gnn in self.layers:
        #     h_dict = gnn(g[0], h_dict)

        # for gnn, block in zip(self.layers, g):
        #     h_dict = gnn(block, h_dict)
...
class HANLayer(nn.Module):
    ...
    def forward(self, g, h):
        ...
        if self._cached_graph is None or self._cached_graph is not g:
            ...
            # g = dgl.heterograph({
            #     ('author', 'ap', 'paper'): g.edges(etype='ap'),
            #     (('field', 'fp', 'paper')): g.edges(etype='fp'),
            #     (('paper', 'pa', 'author')): g.edges(etype='pa'),
            #     (('paper', 'pf', 'field')): g.edges(etype='pf'),
            # })

            self._cached_graph = g
            self._cached_coalesced_graph.clear()
            for mp, mp_value in self.meta_paths_dict.items():
                # temp_g, adj = dgl.metapath_reachable_graph(g, mp_value)
                # self._cached_coalesced_graph[mp] = dgl.to_block(temp_g)
