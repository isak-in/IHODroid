...
class NodeClassification(BaseTask):
    ...
    def __init__(self, args):
        ...
        # 后添加
        # self.outEmbeding = torch.zeros((len(self.test_idx + self.val_idx), 128))
    ...
    def evaluate(self, logits, mode='test', info=True):
        ...
        if self.evaluation_metric == 'acc':
            ...
            # 计算准确率、召回率、宏观F1和微观F1
            # f1_dict = self.evaluator.cal_multi(self.labels[mask], pred)
            return f1_dict
        else:
    ...
    def downstream_evaluate(self, logits, evaluation_metric):
        if evaluation_metric == 'f1_lr':
            ...
            # 自定义添加 Metapathvec，查看嵌入表示
            self.outEmbeding = logits
            # torch.save(self.outEmbeding[torch.cat([self.train_idx, self.val_idx, self.test_idx], dim=0)],
            #            'Metapathvec.embeding.pt')
            #
            # torch.save(self.labels[torch.cat([self.train_idx, self.val_idx, self.test_idx], dim=0)],
            #            'Metapathvec.label.pt')
